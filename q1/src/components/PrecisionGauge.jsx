...
export default function PrecisionGauge({ value, max, label, unit, color, ticks, size = 260, isCenter = false, isAlert = false }) {
  ...
  return (
    <div style={{ ...styles.pod, width: size, height: size, zIndex: isCenter ? 10 : 1 }}>
      ...
      <div style={{ 
        ...styles.podContent, 
        top: isCenter ? "58%" : "55%", // Starts the text block below the center pivot
        justifyContent: "flex-start" 
      }}>
        
        {/* 1. Unit (Top of the safe zone) */}
        <div style={{ 
          color: "#8a939b", 
          fontSize: "12px", 
          fontWeight: 600, 
        ...
        {/* 3. Main Value (Bottom - largest and clearest) */}
        <div style={{ 
          color: isAlert ? "#ef4444" : "#fff", 
          fontSize: isCenter ? "52px" : "34px", 
          fontWeight: 700, 
          lineHeight: 1,
          marginTop: "5px" // Tucked right under the label
        }}>
...
const styles = {
  ...
  podContent: {
    ...
    alignItems: "center",
    // Removed justifyContent: center to allow top: % to work correctly
    zIndex: 5,
  }
};